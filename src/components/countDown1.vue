<template>
  <transition name="bkcd">
    <div class="bkCountDown" v-show="bkCountDownShow">
      ...
      <transition name="loadtip">
        <loading v-show="loadingIsShow" :eleH="eleH" :eleB="eleB" :dataIsOk="dataIsOk" @tipShow="showTips"></loading>
      </transition>
    </div>
  </transition>
</template>
...
//      this.timeInterval -= this.d * this.days
      ...
      showTips (val) {
        this.loadingIsShow = val.loadingIsShow
      }
    },
    components: {
      'Loading': Loading
...
<style>
  *{
  ...
  .loadtip-enter-active, .loadtip-leave-active{
    transition: opacity .5s
  }
  .loadtip-enter, .loadtip-leave-to{
    opacity: 0
  }
  .displayNone{
    display: none;
